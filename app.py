...
def video_analysis():
    ...
    return send_from_directory('.', 'video-analysis.html')
...
def predict_game():
    """Make predictions based on cognitive game performance"""
    try:
        data = request.get_json()
        game_type = data.get('gameType', 'emotion')
        avg_time = float(data.get('averageReactionTime', 0))
        accuracy = float(data.get('accuracy', 0))
        
        risk_score = 0
        description = ""
        
        if game_type == 'emotion':
            # Emotion Recognition: Slower times and lower accuracy = Higher Risk
            # Average neurotypical reaction time is ~1.0s - 1.5s
            time_penalty = max(0, avg_time - 1.5) * 15 # penalty for every second over 1.5s
            accuracy_penalty = (100 - accuracy) * 0.8
            risk_score = min(100, int(time_penalty + accuracy_penalty))
            
            if risk_score >= 60:
                description = 'Your performance correlates with potential challenges in rapid emotion recognition, which can be an indicator of autism spectrum traits.'
            elif risk_score >= 35:
                description = 'Your responses show some delays or inaccuracies in emotion recognition.'
            else:
                description = 'Your emotion recognition speed and accuracy are typical.'
                
        elif game_type == 'visual_search':
            # Visual Pattern Search (Embedded Figures):
            # Individuals with ASD often process details FASTER than neurotypicals.
            # Faster search time (e.g. < 1.0s per pattern) = Higher Risk marker for ASD profile
            if avg_time < 1.0:
                risk_score = 85
                description = 'You demonstrated exceptionally fast detail-focused processing, often associated with the enhanced perceptual functioning seen in autism profiles.'
            elif avg_time < 1.5:
                risk_score = 50
                description = 'You showed strong visual search skills with faster than average performance.'
            else:
                risk_score = 15
                description = 'Your visual search speed is typical, indicating global processing rather than hyper-focused local processing.'
                
        elif game_type == 'go_nogo':
            # Go/No-Go Task (Inhibition & Executive Function):
            # Lower accuracy (high False Alarms) indicates impulsivity/executive dysfunction (ADHD/ASD overlap).
            error_rate = 100 - accuracy
            risk_score = min(100, int(error_rate * 2.5))
            
            if risk_score >= 60:
                description = 'High rates of false alarms indicate potential challenges with response inhibition and sustained attention, common in neurodivergent profiles.'
            elif risk_score >= 35:
                description = 'Moderate number of impulsive responses detected.'
            else:
                description = 'Strong impulse control and sustained attention demonstrated.'
                
        else:
            risk_score = 0
            description = 'Unknown game type submitted.'
